...
std::wstring narrow_to_wide(const std::string& input) {
    ...
    if (count <= 0) return std::wstring(input.begin(), input.end());
...
std::wstring make_command_line(const LaunchOptions& options) {
    ...
    for (const auto& arg : options.gameArguments) cmd << L' ' << Launcher::quote(arg);
    return cmd.str();
}
...
bool Launcher::inject_client(void* processHandleRaw, const std::wstring& clientDll, unsigned long& loadResult) const {
    HANDLE process = static_cast<HANDLE>(processHandleRaw);
    const std::size_t bytes = (clientDll.size() + 1) * sizeof(wchar_t);
    ...
    SIZE_T written = 0;
    if (!WriteProcessMemory(process, remoteString, clientDll.c_str(), bytes, &written) || written != bytes) {
    ...
    const auto loadLibraryW = reinterpret_cast<LPTHREAD_START_ROUTINE>(GetProcAddress(kernel32, "LoadLibraryW"));
    if (!loadLibraryW) {
        VirtualFreeEx(process, remoteString, 0, MEM_RELEASE);
        return false;
    }

    HANDLE thread = CreateRemoteThread(process, nullptr, 0, loadLibraryW, remoteString, 0, nullptr);
    if (!thread) {
    ...
    DWORD exitCode = 0;
    const bool ok = GetExitCodeThread(thread, &exitCode) && exitCode != 0;
    if (!ok) std::cerr << "LoadLibraryW failed inside RDR.exe.\n";
    loadResult = exitCode;
...
std::vector<wchar_t> Launcher::build_environment(const LaunchOptions& options) {
    ...
    if (raw) {
        ...
        while (*cursor != L'\0') {
            ...
            const std::wstring key = eq == std::wstring::npos ? entry : entry.substr(0, eq);
    ...
    entries.push_back(L"FRONTIER_SERVER=" + narrow_to_wide(options.serverHost));
    entries.push_back(L"FRONTIER_PORT=" + std::to_wstring(options.serverPort));
    entries.push_back(L"FRONTIER_NAME=" + narrow_to_wide(options.playerName));
...
int Launcher::run(const LaunchOptions& options) const {
    std::error_code ec;
    const auto gamePath = std::filesystem::weakly_canonical(options.gameExe, ec);
    ...
    ec.clear();
    const auto dllPath = std::filesystem::weakly_canonical(options.clientDll, ec);
    if (ec || !std::filesystem::exists(dllPath)) {
        std::wcerr << L"FrontierClient.dll not found: " << options.clientDll << L"\n";
        return 2;
    }

    if (!verify_game(gamePath.wstring())) return 3;

    std::wstring commandLine = make_command_line(LaunchOptions{
        gamePath.wstring(), dllPath.wstring(), options.serverHost,
        options.serverPort, options.playerName, options.gameArguments
    });
    ...
    auto environment = build_environment(options);

    if (!CreateProcessW(gamePath.c_str(), mutableCommand.data(), nullptr, nullptr, FALSE,
                         CREATE_SUSPENDED | CREATE_UNICODE_ENVIRONMENT, environment.data(),
                         gamePath.parent_path().c_str(), &startup, &processInfo)) {
        std::wcerr << L"CreateProcessW failed: " << GetLastError() << L"\n";
        return 4;
    }

    unsigned long loadResult = 0;
    if (!inject_client(processInfo.hProcess, dllPath.wstring(), loadResult)) {
    ...
    std::cout << "FrontierClient.dll loaded successfully.\n";
