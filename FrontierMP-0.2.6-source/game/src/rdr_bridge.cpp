...
#ifdef _WIN32
                        // Read-only diagnostic: verify whether the returned Actor handle is present
                        // in the same actor-manager backing storage used by the local-player chain.
                        std::uintptr_t managerSlots = 0;
                        if (read_pointer(actorManagerSlotsStorage_, managerSlots)) {
                            const auto slotIndex = static_cast<std::uint16_t>(actorHandle);
                            const auto slotAddress = managerSlots +
                                (static_cast<std::uintptr_t>(slotIndex) * 0x10u);
                            std::uintptr_t managerActor = 0;
                            const bool slotRead = read_pointer(slotAddress, managerActor);

                            char managerBuffer[384]{};
                            std::snprintf(managerBuffer, sizeof(managerBuffer),
                                          "[FrontierRemoteActor] manager-resolve handle=0x%08X index=0x%04X slots=0x%llX slot=0x%llX read=%u actor=0x%llX",
                                          actorHandle,
                            ...
                            write_bridge_log_line(managerBuffer);

                            if (slotRead && managerActor != 0 &&
                                readable(managerActor, 0x200)) {
                                constexpr float kPositionEpsilon = 0.01f;
                                const auto close = [](float a, float b) {
                                    return std::fabs(a - b) <= kPositionEpsilon;
                                };

                                bool chainHit = false;
                                std::uintptr_t hitComponent = 0;
                                std::uintptr_t hitTransform = 0;
                                std::size_t hitActorOffset = 0;

                                // Diagnostic only. We already know the local chain uses:
                                // actor + 0xB0 -> component, component + 0x18 -> transform,
                                // transform + 0x30 -> position. Search a bounded actor window
                                // instead of assuming those offsets are universal for remotes.
                                for (std::size_t actorOffset = 0;
                                     actorOffset <= 0x1F8u && !chainHit;
                                     actorOffset += sizeof(std::uintptr_t)) {
                                    std::uintptr_t componentCandidate = 0;
                                    if (!guarded_read_pointer(managerActor + actorOffset, componentCandidate) ||
                                        componentCandidate == 0 ||
                                        !readable(componentCandidate, 0x20)) {
                                        continue;
                                    }

                                    std::uintptr_t transformCandidate = 0;
                                    if (!guarded_read_pointer(componentCandidate + 0x18u, transformCandidate) ||
                                        transformCandidate == 0 ||
                                        !readable(transformCandidate + 0x30u, sizeof(Vec3))) {
                                        continue;
                                    }
                                    ...
                                    chainHit = true;
                                    hitComponent = componentCandidate;
                                    hitTransform = transformCandidate;
                                    hitActorOffset = actorOffset;
                                }
                                ...
                                if (chainHit) {
                                    ...
                                    std::snprintf(hitBuffer, sizeof(hitBuffer),
                                                  ...
                                                  x, y, z);
                                    write_bridge_log_line(hitBuffer);
                                } else {
                                    write_bridge_log_line(
                                        "[FrontierRemoteActor] chain-miss no actor->component->transform candidate matched spawn position");
                                }
                            }
                        } else {
                            write_bridge_log_line(
                                "[FrontierRemoteActor] manager-resolve slots-read-failed");
                        }
#endif
